...
export interface SeoData {
  ...
  canonical?:   string;   // full URL override
}
...
export class SeoService {
  ...
  static readonly DEF_DESC = 'Nowara Infotech â€” Certified Microsoft Dynamics 365 ERP Partner delivering Business Central, F&O, CRM, LS Central, Power BI and Cloud across UAE & India.';
  ...
  update(data: SeoData, routePath = '') {
    ...
    this.metaSv.updateTag({ name: 'robots',      content: 'index, follow'  });
    if (data.keywords) {
  ...
  setPage(title: string, description: string, path = '') {
    this.update({ title, description }, path);
  }
}
